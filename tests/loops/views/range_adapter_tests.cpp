...
TEST(loops_view_tests, range_adaptor_can_process_loops) {
  int test[5] = {10, 11, 12, 13, 14};
  loops::enumerate(loops::range(loops::just(test)),
                   [&](auto idx, auto value) { ASSERT_EQ(test[idx], value); });
}

TEST(loops_view_tests, range_adaptor_can_process_loops_pipe) {
  int test[5] = {10, 11, 12, 13, 14};
  loops::enumerate(test | loops::just | loops::range,
                   [&](auto idx, auto value) { ASSERT_EQ(test[idx], value); });
}

TEST(loops_view_tests, range_adaptor_supports_for_loop) {
  int test[5] = {10, 11, 12, 13, 14};
...
TEST(loops_view_tests, range_adaptor_supports_for_loop_with_pipe) {
  int test[5] = {10, 11, 12, 13, 14};
  int idx = 0;
